...
def create_radar_VIN_msg(radarId, radarVIN, radarCAN, radarTriggerMessage, useRadar, radarPosition, radarEpasType):
  ...
  if radarId == 0:
    print(type(useRadar))
    print(type(radarPosition))
    print(type(radarEpasType))
    dat1 = int(useRadar) + int(radarPosition << 1) + int(radarEpasType << 3)
    struct.pack_into('BBBBBBBB', msg, 0, radarId, radarCAN, dat1, int((radarTriggerMessage >> 8) & 0xFF), int(radarTriggerMessage & 0xFF), ord(radarVIN[0]), ord(radarVIN[1]), ord(radarVIN[2]))
  if radarId == 1:
